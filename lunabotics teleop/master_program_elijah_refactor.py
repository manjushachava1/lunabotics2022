...
class SpeedControl: #contains all relevant information to control the speeds; should encapsulate all the button statements besides 296 and 297
    ...
    # does a simple matching search to check if the scancode corresponds to a positive or negative change,
    # and a second to determine what speed is changing. should be faster because once it finds its
    # target it stops searching
    def speed_adjust(self, scancode): 
...
#buttons, two change which servo is getting input, and the rest adjust speed
    if event.type == ecodes.EV_KEY:
        ...
        if keyevent.keystate == KeyEvent.key_down: #not sure if this is redundant with ecodes.EV_KEY, left it in just in case
            ...
            elif keyevent.scancode in robot.speed.speed_scancodes:
                print("Receiving speed adjust order")
